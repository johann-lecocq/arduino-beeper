...
INCLUDES = -I"/usr/lib/avr/include/avr" -I"./include" -I"./lib" #{Headers.join(" ")}
DEFINES = -DF_CPU=$(F_CPU)L -DARDUINO=100
...
\t$(SIZE) $(OUTPUT).hex | ruby scripts/size.rb $(MAX_SIZE) "$(OUTPUT).hex"
  
.PHONY: upload clean

upload: all
\tstty -F $(PORT) hupcl
\t$(AVRDUDE) -p$(MCU) -carduino -C/etc/avrdude.conf -P$(PORT) -b$(UPLOAD_RATE) -D -Uflash:w:$(OUTPUT).hex:i 
